...
class App {
  ...
  init(data) {
    ...
    if (historyData) {
      this.curQuesIndex = +localStorage.getItem('historyCurIndex');
      this.quesData = JSON.parse(historyData);
    } else {
      this.curQuesIndex = 0;

      this.quesData = [...data];
      this.quesData.sort(() => Math.random() - 0.5);

      localStorage.setItem('historyData', JSON.stringify(this.quesData));
      localStorage.setItem('historyCurIndex', this.curQuesIndex);
    }
  }
  ...
  loadQues() {
    const container = this.createElement('div', ['main__wrapper'], '');

    const ques = this.quesData[this.curQuesIndex];

    const numberQuesElement = this.createElement(
      'p',
      ['question__number'],
      this.curQuesIndex + 1
    );
    ...
    const backBtn = this.createElement('button', ['btn', 'btn-back'], 'Back');
    backBtn.addEventListener('click', () => this.backQues());
    const nextBtn = this.createElement('button', ['btn', 'next-back'], 'Next');
    nextBtn.addEventListener('click', () => this.nextQues());

    quesWrapper.append(numberQuesElement, quesElement, answerElement);
    container.append(quesWrapper, backBtn, nextBtn);
  ...
  nextQues() {
    ...
    if (this.curQuesIndex >= this.quesData.length) this.curQuesIndex = 0;
    console.log('this.curQuesIndex', this.curQuesIndex);
    this.loadQues();
    this.saveHistoryCurIndex();
  }

  backQues() {
    this.curQuesIndex -= 1;

    if (this.curQuesIndex < 0) this.curQuesIndex = this.quesData.length - 1;
    console.log('this.curQuesIndex', this.curQuesIndex);
    this.loadQues();
    this.saveHistoryCurIndex();
  }
